 ...
 */
@Slf4j
public class WorkYearJob implements Runnable{

    public  void analyse() {
        log.info("开始分析工作年限情况");

        try {
            HbaseClient.truncateTable("workYear");
            ...
            job.setJarByClass(WorkYearJob.class);
            job.setJobName("WorkYearJob");

            job.setMapperClass(WorkYearMapper.class);
...
//        FileInputFormat.addInputPaths(job,"/data/input/wordcount.txt");
            ...
            TableMapReduceUtil.initTableReducerJob("workYear", WorkYearReducer.class, job);
